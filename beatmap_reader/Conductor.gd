...
var next_hit_obj = 0
var warning_time: float = 1.5 # warning time for each hit object in seconds
#var debug = true
var debug = false
...
#	assert(playing, "error: not playing!")
	...
	if hardware_clock:
		...
		song_position = max(new_song_position, song_position)
	else:
		song_position = max((Time.get_ticks_usec() - start_time) / USEC_PER_SEC - time_delay, 0)
		song_position += entry_point / MSEC_PER_SEC
...
func get_song_position(units: int = SEC) -> float:
	match units:
		SEC:
			return song_position
		MSEC:
			return song_position * MSEC_PER_SEC
